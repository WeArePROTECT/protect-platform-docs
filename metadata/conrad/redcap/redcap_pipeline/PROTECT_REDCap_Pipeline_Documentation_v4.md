...
**Last updated:** March 25, 2026 (v4)  
**Pipeline version:** protect_pipeline_v4.py  
**Notebook version:** PROTECT_Data_Integration_v5.ipynb  
...
| Change | Details |
|---|---|
| **LINKAGE_PATH updated to v4_0** | `patient_sputum_asma_gold_linkage_table_v4_0.csv` — replaces v3 |
| **163 omics-only samples added to linkage table** | Emma Rooholfada (Zengler Lab) metaG/metaRS dataset samples registered as sample records without ASMA isolate IDs. These were designated for sequencing only — never sent to Berkeley for bacterial isolation. New rows have `ASMA_id=blank`, `sk_notes='omics_only — added v4.0'`. |
| **Master merged now covers all enrolled samples** | Previously only isolation samples appeared in the linkage table. Now omics-only samples appear too with `has_isolates=False`, enabling full cohort visibility. |
| **Output directory updated to dated run folder** | `data_reception_pipeline_out/3_25_26/` |
| **Output filenames now include run date suffix** | `PROTECT_REDCap_clean_3_25_26.csv`, `PROTECT_REDCap_merged_3_25_26.csv` |
| **`os.makedirs` added to Section 0** | Output directory is now created automatically if it does not exist |
| **Notebook versioned to v5** | `PROTECT_Data_Integration_v5.ipynb` |
| **Pipeline versioned to v4** | `protect_pipeline_v4.py` |
...
**File:** `patient_sputum_asma_gold_linkage_table_v4_0.csv` (updated from v3)  
**Owner:** Berkeley / Arkin Lab (Sun-Young Kim, wet lab)  
**Shape:** ~5,182 rows × 10 columns (5,019 original isolation rows + 163 new omics-only rows)  
...
### Output 1: `PROTECT_REDCap_clean_<DATE>.csv` — Silver Layer
...
### Output 2: `PROTECT_REDCap_merged_<DATE>.csv` — Gold Layer
...
2. Open `PROTECT_Data_Integration_v5.ipynb` and update `REDCAP_RAW_PATH` in Section 0
   - Also update the output directory and filename date suffix to reflect the current run date (e.g. `3_25_26` → `4_15_26`)
...
|---|---|---|---|
| `protect_pipeline_v4.py` | Main pipeline script — run monthly | Berkeley | Update `REDCAP_RAW_PATH` and `APL_METADATA_PATH` at top |
| `PROTECT_Data_Integration_v5.ipynb` | Interactive notebook version | Berkeley | Same logic as pipeline script |
| `PROTECT_REDCap_clean_<DATE>.csv` | Output: silver layer, 1 row/visit | Pipeline output | Overwritten each run |
| `PROTECT_REDCap_merged_<DATE>.csv` | Output: gold layer, 1 row/isolate | Pipeline output | Overwritten each run — now 76 cols |
| `PROTECT_RedCapDataExport_*.csv` | Input: raw REDCap export | UCSD / Dahen | Update path each month |
| `patient_sputum_asma_gold_linkage_table_v4_0.csv` | Input: ASMA isolate + omics sample records | Berkeley / Spencer Long | Updated from v3 — adds 163 omics-only samples |
...
| `PROTECT_Pipeline_Documentation_v4.md` | This document | Berkeley / Spencer | |
